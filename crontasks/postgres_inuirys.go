...
func AddCountVisitOnIURLPerDay(db *sql.DB, ctx context.Context) {
	...
	rows, err := db.QueryContext(ctx, "SELECT count(distinct useragent) as count_unique_visits, count(useragent) as count_vists, shorturl FROM activity WHERE click_time BETWEEN $1 AND $2 GROUP BY shorturl  ORDER BY shorturl ASC;", dateStrat, dateStop)
	if err != nil {
		log.Panic(err)
	}
	defer rows.Close()
