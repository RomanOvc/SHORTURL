...
func (r *AuthInquirysRepository) CreateUser(ctx context.Context, useremail, password string, activate bool) (string, error) {
	...
	// TODO не понимаю зачем это надо обработать
	...
	err = tx.QueryRowContext(ctx, "INSERT INTO users (useremail,password,activate, created_user) values ($1, $2, $3, $4) RETURNING user_id", useremail, password, activate, time.Now()).Scan(&userId)
	if err != nil {
	...
	_, err = tx.ExecContext(ctx, "INSERT INTO emailactivate (uid,user_id,active_until) values ($1, $2,$3)", userUuid, userId, time.Now().Add(time.Hour))
	if err != nil {
		return "", errors.Wrap(err, "err insert into emailactivate")
	}
...
func (r *AuthInquirysRepository) CheckUserUuidToEmail(ctx context.Context, uuid string) error {
	...
	_, err = tx.ExecContext(ctx, "UPDATE users SET activate = $2 WHERE user_id = $1", resUserId, true)
	if err != nil {
